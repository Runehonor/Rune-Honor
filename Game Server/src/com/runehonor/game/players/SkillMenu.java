...
public class SkillMenu {
	...
	public static void openInterface(Client c, int skillType) {
		...
		c.getAssistant().showInterface(INTERFACE_ID);
	}
	...
	private static void removeSidebars(Client c) {
		...
		for (int j = 0; j < temp.length; j++) {
			c.getAssistant().sendFrame126("",temp[j]);
		}
	}
	...
	private static void writeText(Client c, int skillType) {
		c.getAssistant().sendFrame126(SKILLS[skillType], TITLE_LINE);
		for (int j = 0; j < LEVELS[skillType].length; j++) {
			c.getAssistant().sendFrame126(LEVELS[skillType][j], LEVEL_LINE + j);
		}
		for (int j = 0; j < DESCRIPTION[skillType].length; j++) {
			c.getAssistant().sendFrame126(DESCRIPTION[skillType][j], TEXT_LINE + j);
		}
		
		for (int j = DESCRIPTION[skillType].length; j < 30; j++) {
			c.getAssistant().sendFrame126("", LEVEL_LINE + j);
		}
		for (int j = LEVELS[skillType].length; j < 30; j++) {
			c.getAssistant().sendFrame126("", TEXT_LINE + j);
		}
	}
}