...
public class PlayerHandler{
	...
	public static Client[] players = new Client[Constants.MAX_PLAYERS];
	...
	/** 
	 * An integer to increment playercount by.
	 */
	private static int increment = 14;

	public static int getPlayerCount() {
		return playerCount += increment;
	}
	...
	public void updatePlayer(Player plr, Stream str) {
			...
			//castlewars
			if(plr.updateRegion) {
				GameServer.objectManager.loadObjects((Client)plr);
	...
	private void removePlayer(Player plr) {
		if(plr.privateChat != 2) { 
			for(int i = 1; i < Constants.MAX_PLAYERS; i++) {
				...
				Client o = (Client)PlayerHandler.players[i];
				if(o != null) {
					o.getAssistant().updatePM(plr.playerId, 0);
				}
			}
		}
		plr.destruct();
