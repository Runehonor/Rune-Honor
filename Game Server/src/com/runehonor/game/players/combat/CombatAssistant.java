package com.runehonor.game.players.combat;
...
import com.runehonor.game.npcs.KalphiteQueen;
import com.runehonor.game.npcs.NPCHandler;
import com.runehonor.game.players.Client;
import com.runehonor.game.players.PathFinder;
...
public class CombatAssistant{
	...
	*/
			...
			c.followId2 = i;
				...
				if(c.usingMagic) {	// magic hit delay
					...
					if(c.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 78, c.MAGIC_SPELLS[c.spellId][4], getStartHeight(), getEndHeight(), i + 1, 50);
					}
					c.getItems();
					c.hitDelay = getHitDelay(ItemAssistant.getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase());
					c.oldNpcIndex = i;
				...
				if(usingBow && Constants.CRYSTAL_BOW_DEGRADES) { // crystal bow degrading
					...
					if(c.crystalBowArrowCount >= 250){
						switch(c.playerEquipment[c.playerWeapon]) {
							...
							if(!c.getItems().addItem(4207, 1)) {
								GameServer.itemHandler.createGroundItem(c, 4207, c.getX(), c.getY(), 1, c.getId());
							}
	...
	public void delayedHit(int i) { // npc hit delay
		if (NPCHandler.npcs[i] != null) {
			...
			if (NPCHandler.npcs[i].underAttackBy > 0 && GameServer.npcHandler.getsPulled(i)) {
				NPCHandler.npcs[i].killerId = c.playerId;			
			} else if (NPCHandler.npcs[i].underAttackBy < 0 && !GameServer.npcHandler.getsPulled(i)) {
				NPCHandler.npcs[i].killerId = c.playerId;
			}
			c.lastNpcAttacked = i;
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if(c.fightMode == 3) {
					...
					c.getAssistant().addSkillXP((damage*Constants.RANGE_EXP_RATE/3), 4); 
					c.getAssistant().addSkillXP((damage*Constants.RANGE_EXP_RATE/3), 1);				
					c.getAssistant().addSkillXP((damage*Constants.RANGE_EXP_RATE/3), 3);
					c.getAssistant().refreshSkill(1);
					c.getAssistant().refreshSkill(3);
					c.getAssistant().refreshSkill(4);
					
				} else {
					
					c.getAssistant().addSkillXP((damage*Constants.RANGE_EXP_RATE), 4); 
					c.getAssistant().addSkillXP((damage*Constants.RANGE_EXP_RATE/3), 3);
					c.getAssistant().refreshSkill(3);
					c.getAssistant().refreshSkill(4);
				...
				c.getAssistant().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage*Constants.MAGIC_EXP_RATE), 6); 
				c.getAssistant().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage*Constants.MAGIC_EXP_RATE/3), 3);
				c.getAssistant().refreshSkill(3);
				c.getAssistant().refreshSkill(6);
				...
				if(!magicFailed) {
					...
					switch(c.MAGIC_SPELLS[c.oldSpellId][0]) { 
						...
						if(c.playerLevel[3] + heal >= c.getAssistant().getLevelForXP(c.playerXP[3])) {
	...
	public void applyNpcMeleeDamage(int i, int damageMask) {
		...
		boolean fullVeracsEffect = c.getAssistant().fullVeracs() && Misc.random(3) == 1;
		if (NPCHandler.npcs[i].HP - damage < 0) { 
			damage = NPCHandler.npcs[i].HP;
		}
		...
		if(c.fightMode == 3) {
			...
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE/3), 0); 
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE/3), 1);
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE/3), 2); 				
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE/3), 3);
			c.getAssistant().refreshSkill(0);
			c.getAssistant().refreshSkill(1);
			...
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE), c.fightMode); 
			c.getAssistant().addSkillXP((damage*Constants.MELEE_EXP_RATE/3), 3);
			c.getAssistant().refreshSkill(c.fightMode);
			c.getAssistant().refreshSkill(3);
	...
	public void fireProjectileNpc() {
		if(c.oldNpcIndex > 0) {
			if(NPCHandler.npcs[c.oldNpcIndex] != null) {
				...
				int offX = (pY - nY)* -1;
				int offY = (pX - nX)* -1;
				c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, getProjectileSpeed(), getRangeProjectileGFX(), 43, 31, c.oldNpcIndex + 1, getStartDelay());
				if (usingDbow())
					c.getAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, getProjectileSpeed(), getRangeProjectileGFX(), 60, 31,  c.oldNpcIndex + 1, getStartDelay(), 35);
			}
		}
	}
	...
	**/
		...
		if (PlayerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				if(c.usingMagic) {	// magic hit delay
					...
					if(c.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 78, c.MAGIC_SPELLS[c.spellId][4], getStartHeight(), getEndHeight(), -i - 1, getStartDelay());
					}
					if (c.autocastId > 0) {
						c.followId = c.playerIndex;
						c.followDistance = 5;
					...
					if(c.MAGIC_SPELLS[c.oldSpellId][0] == 12891 && o.isMoving) {
						...
						c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 85, 368, 25, 25, -i - 1, getStartDelay());
					}
	...
	public void playerDelayedHit(int i) {
		if (PlayerHandler.players[i] != null) {
			...
			Client o = (Client) PlayerHandler.players[i];
			o.getAssistant().removeAllWindows();
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if(c.fightMode == 3 && c.classType != PlayerClass.IRONMAN) {
				...
				if(!c.magicFailed) {
					if(System.currentTimeMillis() - PlayerHandler.players[i].reduceStat > 35000) {
						...
						switch(c.MAGIC_SPELLS[c.oldSpellId][0]) { 
							...
							case 12999:
							case 13023:
							PlayerHandler.players[i].playerLevel[0] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[0]) * 10) / 100);
							break;
						}
					}
					...
					switch(c.MAGIC_SPELLS[c.oldSpellId][0]) { 	
						...
						case 1153:						
						PlayerHandler.players[i].playerLevel[0] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[0]) * 5) / 100);
						...
						o.getAssistant().refreshSkill(0);
						break;
						
						case 1157:
						PlayerHandler.players[i].playerLevel[2] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[2]) * 5) / 100);
						o.sendMessage("Your strength level has been reduced!");
						PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();						
						o.getAssistant().refreshSkill(2);
						break;
						
						case 1161:
						PlayerHandler.players[i].playerLevel[1] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[1]) * 5) / 100);
						o.sendMessage("Your defence level has been reduced!");
						PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();					
						o.getAssistant().refreshSkill(1);
						break;
						
						case 1542:
						PlayerHandler.players[i].playerLevel[1] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[1]) * 10) / 100);
						o.sendMessage("Your defence level has been reduced!");
						PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] =  System.currentTimeMillis();
						o.getAssistant().refreshSkill(1);
						break;
						...
						case 1543:
						PlayerHandler.players[i].playerLevel[2] -= ((o.getAssistant().getLevelForXP(PlayerHandler.players[i].playerXP[2]) * 10) / 100);
						o.sendMessage("Your strength level has been reduced!");
						PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
						o.getAssistant().refreshSkill(2);
						break;
		...
		if (PlayerHandler.players[playerId] != null) {
			...
			if (checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(mageAtk()) > Misc.random(mageDef()) && !c.magicFailed) {
					...
					c2.getAssistant().refreshSkill(3);
		...
		switch(c.specEffect) {
			...
			if(damage > 0) {
				if(o.prayerActive[16] || o.prayerActive[17] || o.prayerActive[18]) {
					o.headIcon = -1;
					o.getAssistant().sendFrame36(c.PRAYER_GLOW[16], 0);
					o.getAssistant().sendFrame36(c.PRAYER_GLOW[17], 0);
					o.getAssistant().sendFrame36(c.PRAYER_GLOW[18], 0);					
				}
	...
	public void applySmite(int index, int damage) {
		...
		if (PlayerHandler.players[index] != null) { 
			...
			c2.getAssistant().refreshSkill(5);
		}
	...
	public void fireProjectilePlayer() {
		if(c.oldPlayerIndex > 0) {
			if(PlayerHandler.players[c.oldPlayerIndex] != null) {
				...
				int offY = (pX - oX)* -1;	
				if (!c.msbSpec)
					c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, getProjectileSpeed(), getRangeProjectileGFX(), 43, 31, - c.oldPlayerIndex - 1, getStartDelay());
				else if (c.msbSpec) {
					c.getAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, getProjectileSpeed(), getRangeProjectileGFX(), 43, 31, - c.oldPlayerIndex - 1, getStartDelay(), 10);
					c.msbSpec = false;
				}
				if (usingDbow())
					c.getAssistant().createPlayersProjectile2(pX, pY, offX, offY, 50, getProjectileSpeed(), getRangeProjectileGFX(), 60, 31, - c.oldPlayerIndex - 1, getStartDelay(), 35);
			}
		}
	}
	...
	public void activatePrayer(int i) {
		if(c.duelRule[7]){
			for(int p = 0; p < c.PRAYER.length; p++) { // reset prayer glows 
				...
				c.getAssistant().sendFrame36(c.PRAYER_GLOW[p], 0);	
			}
		...
		if(c.playerLevel[5] > 0 || !Constants.PRAYER_POINTS_REQUIRED){
			if(c.getAssistant().getLevelForXP(c.playerXP[5]) >= c.PRAYER_LEVEL_REQUIRED[i] || !Constants.PRAYER_LEVEL_REQUIRED) {
				...
				switch(i) {
					...
					if(c.prayerActive[i] == false) {
						for (int j = 0; j < defPray.length; j++) {
							if (defPray[j] != i) {
								...
								c.getAssistant().sendFrame36(c.PRAYER_GLOW[defPray[j]], 0);
							}								
						}
					}
					break;
					...
					if(c.prayerActive[i] == false) {
						for (int j = 0; j < strPray.length; j++) {
							if (strPray[j] != i) {
								...
								c.getAssistant().sendFrame36(c.PRAYER_GLOW[strPray[j]], 0);
							}								
						}
						for (int j = 0; j < rangePray.length; j++) {
							if (rangePray[j] != i) {
								c.prayerActive[rangePray[j]] = false;
								c.getAssistant().sendFrame36(c.PRAYER_GLOW[rangePray[j]], 0);
							}								
						}
						for (int j = 0; j < magePray.length; j++) {
							if (magePray[j] != i) {
								c.prayerActive[magePray[j]] = false;
								c.getAssistant().sendFrame36(c.PRAYER_GLOW[magePray[j]], 0);
							}								
						}
					}
					break;
					...
					if(c.prayerActive[i] == false) {
						for (int j = 0; j < atkPray.length; j++) {
							if (atkPray[j] != i) {
								...
								c.getAssistant().sendFrame36(c.PRAYER_GLOW[atkPray[j]], 0);
							}								
						}
						for (int j = 0; j < rangePray.length; j++) {
							if (rangePray[j] != i) {
					...
					if(System.currentTimeMillis() - c.stopPrayerDelay < 5000) {
						...
						c.getAssistant().sendFrame36(c.PRAYER_GLOW[16], 0);
						c.getAssistant().sendFrame36(c.PRAYER_GLOW[17], 0);
						c.getAssistant().sendFrame36(c.PRAYER_GLOW[18], 0);
						return;
					}
					if (i == 16)
				...
				if(!headIcon) {
					if(c.prayerActive[i] == false) {
						...
						c.getAssistant().sendFrame36(c.PRAYER_GLOW[i], 1);					
					} else {
						c.prayerActive[i] = false;
						c.getAssistant().sendFrame36(c.PRAYER_GLOW[i], 0);
					}
				} else {
					if(c.prayerActive[i] == false) {
						c.prayerActive[i] = true;
				...
				c.getAssistant().sendFrame36(c.PRAYER_GLOW[i],0);
				c.getAssistant().sendFrame126("You need a @blu@Prayer level of "+c.PRAYER_LEVEL_REQUIRED[i]+" to use "+c.PRAYER_NAME[i]+".", 357);
	...
	public void activateSpecial(int weapon, int i){
		...
		case 10887:
			...
			case 1249:
				...
				if (c.playerIndex > 0) {
					Client o = (Client)PlayerHandler.players[i];
					o.getAssistant().getSpeared(c.absX, c.absY);
				}	
			break;
	...
	**/
	...
	public int calculateMeleeMaxHit() {
		...
		if(c.playerEquipment[c.playerWeapon] == 4718 && c.playerEquipment[c.playerHat] == 4716 && c.playerEquipment[c.playerChest] == 4720 && c.playerEquipment[c.playerLegs] == 4722) {	
				maxHit += (c.getAssistant().getLevelForXP(c.playerXP[3]) - c.playerLevel[3]) / 2;			
		}
		if (c.specDamage > 1)
	...
	**/
	...
	public boolean checkMagicReqs(int spell) {
		...
		if(c.usingMagic && staffRequired > 0 && Constants.RUNES_REQUIRED) { // staff required
