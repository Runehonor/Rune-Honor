...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
		...
		if (System.currentTimeMillis() - c.potDelay >= 1200) {
			...
			c.attackTimer++;
            c.sendMessage("You drink some of your "+ com.runehonor.game.items.Item.getItemName(itemId) +".");
            String item =  com.runehonor.game.items.Item.getItemName(itemId);
	...
	public void drinkSlayerPotion(int itemId, int replaceItem, int slot, int stat, boolean sup) {
		...
		c.getAssistant().refreshSkill(Player.playerSlayer);
		c.playerLevel[Player.playerAttack] -= 3;
		c.getAssistant().refreshSkill(Player.playerAttack);
		c.playerLevel[Player.playerStrength] -= 3;
		c.getAssistant().refreshSkill(Player.playerStrength);
	}
	...
	public void enchanceMagic(int skillID, boolean sup) {
		...
		c.getAssistant().refreshSkill(skillID);
	}
	...
	public void restoreStats() {
		for (int j = 0; j <= 6; j++) {
			...
			if (c.playerLevel[j] < c.getLevelForXP(c.playerXP[j])) {
				...
				c.getAssistant().refreshSkill(j);
				c.getAssistant().setSkillLevel(j, c.playerLevel[j], c.playerXP[j]);
			}			
		}
	}
	...
	public void doTheBrew(int itemId, int replaceItem, int slot) {
		...
		for (int tD : toDecrease) {
			...
			if (c.playerLevel[tD] < 0)
				c.playerLevel[tD] = 1;
			c.getAssistant().refreshSkill(tD);
			c.getAssistant().setSkillLevel(tD, c.playerLevel[tD], c.playerXP[tD]);
		}
