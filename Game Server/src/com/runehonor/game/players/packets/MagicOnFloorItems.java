...
public class MagicOnFloorItems implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		if(!GameServer.itemHandler.itemExists(itemId, itemX, itemY)) {
			c.stopMovement();
			return;
		}
		c.usingMagic = true;
		...
		if(c.goodDistance(c.getX(), c.getY(), itemX, itemY, 12)) {
			...
			c.getAssistant().createPlayersStillGfx(144, itemX, itemY, 0, 72);
			c.getAssistant().createPlayersProjectile(c.getX(), c.getY(), offX, offY, 50, 70, c.MAGIC_SPELLS[51][4], 50, 10, 0, 50);
			
			c.getAssistant().addSkillXP(c.MAGIC_SPELLS[51][7], 6);
