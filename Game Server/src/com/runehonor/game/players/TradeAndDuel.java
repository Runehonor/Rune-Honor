...
public class TradeAndDuel{
	...
	**/
	...
	public void requestTrade(int id){
		try {
			...
			if (c.classType == PlayerClass.IRONMAN) { 
				c.sendMessage(IronMan.ironTrade);
				return;
			}
			if (o.classType == PlayerClass.IRONMAN) { 
				c.sendMessage("You cannot trade with Iron Men.");
				return;
			}
			if(o.playerIsBusy()) {
	...
	public void openTrade() {
		...
		c.getAssistant().sendFrame126("Trading with: " + Misc.capitalize(c.playerName)+" " ,3417);
		c.getAssistant().sendFrame126("", 3431);
		c.getAssistant().sendFrame126("Are you sure you want to make this trade?", 3535);
		c.getAssistant().sendFrame248(3323, 3321);
	}	
		...
		if(amount > c.playerItemsN[fromSlot])
			...
			if(!Item.itemStackable[itemID]) {
				for(int a = 0; a < amount; a++) {
					for (GameItem item : offeredItems) {
						if(item.id == itemID) {	
							if(!item.stackable) {	
								...
								o.getAssistant().sendFrame126("Trading with: " + Misc.capitalize(c.playerName)+"", 3417);		
							} else {
					...
					c.getAssistant().sendFrame126("", 3431);
					o.getAssistant().sendFrame126("", 3431);
					}
				}	
			}
			for (GameItem item : offeredItems) {
	...
	public void declineTrade(boolean tellOther) {
		...
		if(tellOther){
			...
			o.getTradeAndDuel().c.getAssistant().removeAllWindows();
		}
	...
	public void confirmScreen() {
		...
		c.getAssistant().sendFrame126(SendTrade, 3557);
		...
		c.getAssistant().sendFrame126(SendTrade, 3558);
		//TODO: find out what 197 does eee 3213
		c.getAssistant().sendFrame248(3443, 197);
	}
	...
	**/
	...
	public void openDuel() {
		...
		c.getAssistant().sendFrame126("Dueling with: " + Misc.capitalize(o.playerName) + " (level-" + o.combatLevel + ")", 6671);
		c.getAssistant().sendFrame126("", 6684);
		c.getAssistant().sendFrame248(6575, 3321);
		c.getItems().resetItems(3322);
	}
	...
	public void refreshduelRules() {
		...
		c.getAssistant().sendFrame87(286, 0);
		c.duelOption = 0;
	}
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		if (o.classType == PlayerClass.IRONMAN) { 
			c.sendMessage("That player is an Iron Man and cannot accept staked items.");
			return false;
		}
		if(!c.openDuel && !o.openDuel) {
			declineDuel();
		...
		if(!Item.itemStackable[itemID]) {
			...
			c.getAssistant().sendFrame126("", 6684);
			o.getAssistant().sendFrame126("", 6684);
		}
	...
	public void confirmDuel() {
		...
		c.getAssistant().sendFrame126(itemId, 6516);
		itemId = "";
		for(GameItem item : o.getTradeAndDuel().stakedItems) {
			if(Item.itemStackable[item.id] || Item.itemIsNote[item.id]) {
				c.getItems();
		...
		c.getAssistant().sendFrame126(itemId, 6517);
		c.getAssistant().sendFrame126("", 8242);
		for(int i = 8238; i <= 8253; i++) {
			c.getAssistant().sendFrame126("", i);
		}
		c.getAssistant().sendFrame126("Hitpoints will be restored.", 8250);
		c.getAssistant().sendFrame126("Boosted stats will be restored.", 8238);
		if(c.duelRule[8]) {
			c.getAssistant().sendFrame126("There will be obstacles in the arena.", 8239);
		} 
		c.getAssistant().sendFrame126("", 8240);
		c.getAssistant().sendFrame126("", 8241);
		...
		for(int i = 0; i < 8; i++) {
			if(c.duelRule[i]) {
				c.getAssistant().sendFrame126(""+rulesOption[i], lineNumber);
				lineNumber ++;
			}
		}
		c.getAssistant().sendFrame126("", 6571);
		c.getAssistant().sendFrame248(6412, 197);
	...
	public void startDuel() {
		...
		if(c.duelRule[8]){	
			if(c.duelRule[1]) {
				c.getAssistant().movePlayer(c.duelTeleX, c.duelTeleY, 0);
			} else {
				c.getAssistant().movePlayer(3366 + Misc.random(12), 3246 + Misc.random(6), 0);
			}
		} else {
			if(c.duelRule[1]) {
				c.getAssistant().movePlayer(c.duelTeleX, c.duelTeleY, 0);
			} else {	
				c.getAssistant().movePlayer(3335 + Misc.random(12), 3246 + Misc.random(6), 0);
			}
		}

		c.getAssistant().createPlayerHints(10, o.playerId);
		c.getAssistant().showOption(3, 0, "Attack", 1);
		for (int i = 0; i < 20; i++) {
			c.playerLevel[i] = c.getAssistant().getLevelForXP(c.playerXP[i]);
	...
	public void duelVictory() {
		Client o = (Client) PlayerHandler.players[c.duelingWith];
		if(o != null) {
			c.getAssistant().sendFrame126(""+o.combatLevel, 6839);
			c.getAssistant().sendFrame126(o.playerName, 6840);
			o.duelStatus = 0;
		} else {
			c.getAssistant().sendFrame126("", 6839);
			c.getAssistant().sendFrame126("", 6840);
		}
		c.duelStatus = 6;
		...
		duelRewardInterface();
		c.getAssistant().showInterface(6733);
		...
		c.getAssistant().requestUpdates();
		c.getAssistant().showOption(3, 0, "Challenge", 3);
		c.getAssistant().createPlayerHints(10, -1);
		c.canOffer = true;
		c.duelSpaceReq = 0;
		c.openDuel = false;
		o.openDuel = false;
			...
			if(item.id > 0 && item.amount > 0) {
				if(Item.itemStackable[item.id]) {
					if(!c.getItems().addItem(item.id, item.amount)) {
						GameServer.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), item.amount, c.getId());
					}
				} else {
					int amount = item.amount;
					for(int a = 1; a <= amount; a++) {
						if(!c.getItems().addItem(item.id, 1)) {
							GameServer.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), 1, c.getId());
						}
					}
				}
			}	
	...
	public void selectRule(int i) { // rules
		...
		c.getAssistant().sendFrame87(286, c.duelOption);
		o.duelOption = c.duelOption;
		o.duelRule[i] = c.duelRule[i];
		o.getAssistant().sendFrame87(286, o.duelOption);
