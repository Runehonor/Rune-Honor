...
public class Trawler extends GroupMinigame {
	...
	public void resetWalls() {
		for(Wall w : Wall.values()) {
			if(w != null) {
				GameServer.objectManager.removeGameObjects(2);
				GameServer.objectHandler.removeObject(GameServer.objectHandler.getObjectByPosition(w.x, w.y));
	...
	public void updateWall(int index) {
		...
		GameServer.objectHandler.removeObject(GameServer.objectHandler.getObjectByPosition(w.x, w.y));
		GameServer.objectHandler.removeObject(GameServer.objectHandler.getObjectByPosition(w.x + (isSunk ? -128 : 128), w.y));
			...
			if (p != null) {
				p.asClient()
						...
						.sendFrame126(net_ripped ? "@red@Ripped" : "@gre@Okay",
								11935);
				p.asClient().getAssistant().sendFrame126("" + fish_caught, 11937);
				p.asClient().getAssistant().sendFrame126(game_time + " mins", 11938);
				p.asClient().getAssistant().sendFrame20(391, water_level);
			} else {
	...
	public void onStart() {
		...
		for (Player p : players) {
			if (p != null) {
				p.asClient().getAssistant().removeAllSidebars();
				p.asClient().getAssistant().sendMapState(2);
				p.asClient().getAssistant().showInterface(3281);
				p.asClient().getAssistant().sendFrame36(75, 11);
				p.asClient().getAssistant().movePlayer(1885, 4825, 1);
				p.asClient().getAssistant().sendFrame126("", 11936);
			}
		}
		CycleEventHandler.addEvent(this, new CycleEvent() {
			...
			public void stop() {
				for (Player p : players) {
					if (p != null) {
						p.asClient().getAssistant().setSidebarInterfaces(p.asClient(), true);
						p.asClient().getAssistant().showInterface(5596);
						p.asClient().getAssistant().sendMapState(0);
					}
				}
				started = true;
				startGameTimer();
	...
	public void upLadder(Player p, int obX, int obY) {
		if (doAction(p)) {
			if (!isSunk) {
				...
				p.asClient().getAssistant()
						.movePlayer(obX == 1884 ? 1885 : 1892, obY, 1);
			} else {
				p.startAnimation(climb_up);
				p.asClient().getAssistant().movePlayer(obX == 2021 ? 2020 : 2013, obY, 1);
			}
		}
	}
	...
	public void downLadder(Player p, int obX, int obY) {
		if (doAction(p)) {
			if (!isSunk) {
				...
				p.startAnimation(climb_down);
				p.asClient().getAssistant().movePlayer(obX == 2021 ? 2020 : 2013, obY, 0);
			}
		}
	}
	...
	public void setSwimmingAnimations() {
		for(Player p : players) {
			if (p != null) {
				...
				p.asClient().getAssistant().requestUpdates();
				p.asClient().getAssistant().closeAllWindows();
			}
		}
	}
	...
	public void movePlayersLoss() {
		for (Player p : players) {
			if (p != null) {
				p.asClient().getAssistant().movePlayer(1952, 4826, 0);
			}
		}
		in_progress = false;
		game_time = 0;
	...
	public void movePlayerWin(final ArrayList<Player> pl) {
		for (Player p : pl) {
			if (p != null) {
				...
				p.asClient().getAssistant().sendMapState(2);
				p.asClient().getAssistant().showInterface(3281);
				p.asClient().getAssistant().sendFrame36(75, 12);
				p.asClient().getAssistant().movePlayer(2666, 3161, 0);
	...
	public ArrayList<GameItem> playerReward(Player p) {
		...
		p.asClient().getAssistant().addSkillXP(xpToAdd, Player.playerFishing);
		return toReturn;
	}
	...
	public void resetRewardsInterface(Client c) {
		for(int j = 0; j < 45; j++) {
			c.getAssistant().sendFrame34(-1, j, 4640, -1);
		}
	}
	public void showReward(Client c) {
		resetRewardsInterface(c);
		c.inFishingTrawlerRewardsInterface = true;
		c.getAssistant().showInterface(4564);
		for(int j = 0; j < c.fishingTrawlerReward.size(); j++) {
			c.getAssistant().sendFrame34(c.fishingTrawlerReward.get(j).id, j, 4640, c.fishingTrawlerReward.get(j).amount);
		}
	}
	
	public void updateRewardSlot(Client c, int slot) {
		c.inFishingTrawlerRewardsInterface = true;
		c.getAssistant().sendFrame34(c.fishingTrawlerReward.get(slot).id, slot, 4640, c.fishingTrawlerReward.get(slot).amount);
		if(slot != 4 && c.fishingTrawlerReward.size() == 5) {
			c.getAssistant().sendFrame34(c.fishingTrawlerReward.get(4).id, 4, 4640, c.fishingTrawlerReward.get(4).amount);
		}
	}
