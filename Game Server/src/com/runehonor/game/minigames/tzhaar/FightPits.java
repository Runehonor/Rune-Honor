package com.runehonor.game.minigames.tzhaar;
...
public class FightPits {
	...
	public void updateWaitRoom() {
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if (c.getAssistant().inPitsWait()) {
					c.getAssistant().sendFrame126("Next Game Begins In : " + ((gameStartTimer * 3) + (gameTime * 3)) + " seconds.", 6570);
					c.getAssistant().sendFrame126("Champion: " + pitsChampion, 6572);
					c.getAssistant().sendFrame126("", 6664); 
					c.getAssistant().walkableInterface(6673);
				}	
			}	
		}	
	}
	...
	public void removePlayerFromPits(int playerId) {
		for (int j = 0; j < playerInPits.length; j++) {
			if (playerInPits[j] == playerId) {
				...
				c.getAssistant().movePlayer(2399, 5173, 0);
				playerInPits[j] = -1;
				playersRemaining--;
				c.inPits = false;
				break;
	...
	public void addToPitsGame(int playerId) {
		...
		c.getAssistant().walkableInterface(-1);
		playerInPits[pitsSlot++] = playerId;
		c.getAssistant().movePlayer(2392 + Misc.random(12), 5139 + Misc.random(25), 0);
		c.inPits = true;		
	}
}