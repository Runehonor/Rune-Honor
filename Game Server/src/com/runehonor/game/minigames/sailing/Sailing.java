package com.runehonor.game.minigames.sailing;
...
public class Sailing {
	...
	public static void startTravel(final Client player, final int i) {
		player.getAssistant().showInterface(3281);
		player.getAssistant().sendFrame36(75, i);
		player.getAssistant().sendFrame99(2);
		player.getAssistant().movePlayer(0, 0, 0);
		player.canWalk = false;
		if(Misc.random(20) <= 1) {
			player.getAssistant().sendFrame36(75, -1);
			player.getAssistant().sendFrame99(0);
			player.canWalk = true;
			player.getAssistant().closeAllWindows();
			player.getAssistant().underWaterTele();
			return;
		}
		CycleEventHandler.addEvent(player, new CycleEvent() {
			@Override //player.underWater();
			public void execute(CycleEventContainer container) {
				player.getAssistant().movePlayer(getX(i), getY(i), 0);
				player.getAssistant().sendFrame36(75, -1);
				player.getAssistant().sendFrame99(0);
				player.canWalk = true;
				player.getAssistant().closeAllWindows();
