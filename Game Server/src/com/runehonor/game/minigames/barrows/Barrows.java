package com.runehonor.game.minigames.barrows;
	...
	public static void objectAction(Client c, int object) {
		for(int i = 0; i < BarrowsData.PLAYER_EXIT.length; i++) {
			if(BarrowsData.PLAYER_EXIT[i][0] == object) {
				c.getAssistant().movePlayer(BarrowsData.PLAYER_EXIT[i][1],BarrowsData.PLAYER_EXIT[i][2],0);
				//refreshBrothers(c);
				break;
			}
		}
		for(int i = 0; i < BarrowsData.SARCOPHAGUS.length; i++) {
			if(BarrowsData.SARCOPHAGUS[i] == object) {
				...
				GameServer.npcHandler.spawnNpc(c, BarrowsData.BARROW_BROTHER[i][0], BarrowsData.BROTHER_SPAWN[i][0], BarrowsData.BROTHER_SPAWN[i][1], c.heightLevel, 0, BarrowsData.BARROW_BROTHER[i][1], BarrowsData.BARROW_BROTHER[i][2], BarrowsData.BARROW_BROTHER[i][3], BarrowsData.BARROW_BROTHER[i][4], true, true);
		...
		for(int i = 0; i < BarrowsData.DOORS.length; i++) {
			if(BarrowsData.DOORS[i] == object) {
				c.getAssistant().moveThroughDoor(c, object, BarrowsData.DOORS_TYPE[i]);
			}
		}
	}
