package com.runehonor.game.minigames.treasuretrails;
...
public class TreasureTrails {
	...
	public void clueReward(int i1, int a1, int i2, int a2, int i3, int a3,
			int i4, int a4) {
		c.getAssistant().showInterface(6960);
		c.getAssistant().sendFrame34(6963, i1, 0, a1);
		c.getAssistant().sendFrame34(6963, i2, 1, a2);
		c.getAssistant().sendFrame34(6963, i3, 2, a3);
		c.getAssistant().sendFrame34(6963, i4, 3, a4);
	...
	public void Clue(String a, String b, String c1, String d, String e,
		...
		c.getAssistant().sendFrame126(a, 6968);
		c.getAssistant().sendFrame126(b, 6969);
		c.getAssistant().sendFrame126(c1, 6970);
		c.getAssistant().sendFrame126(d, 6971);
		c.getAssistant().sendFrame126(e, 6972);
		c.getAssistant().sendFrame126(f, 6973);
		c.getAssistant().sendFrame126(g, 6974);
		c.getAssistant().sendFrame126(h, 6975);
		c.getAssistant().showInterface(6965);
	}

	public void showitem(int item, String s1) {
		c.getAssistant().sendFrame246(11860, 150, item);
		c.getAssistant().sendFrame126(s1, 11861);
		c.getAssistant().sendFrame164(11859);
	}
	...
	public void Hints(int puzzle) {
		...
		case 1:
			for (int i = 2749; i < 2773; i++) {
				c.getAssistant().sendFrame34P2(i, v, 6985, 1);
				v += 1;
			}
			c.getAssistant().sendFrame34P2(2753, 4, 6985, 1);
			break;
		case 3:
			v = 0;
			...
			c.getAssistant().sendFrame34P2(3623, 4, 6985, 1);
			break;
		case 2:
			v = 0;
			...
			c.getAssistant().sendFrame34P2(3647, 4, 6985, 1);
			break;
		}
	}
	...
	public void openPuzzle(int puzzle) {
		...
		case 1:
			for (int i = 0; i < 24; i++) {
				...
				if (castlePuzzle[d] != 0) {
					c.getAssistant().sendFrame34P2(castlePuzzle[d], v, 6980, 1); // 11126
		...
		case 2:
			for (int i = 0; i < 24; i++) {
				...
				if (trollPuzzle[d] != 0) {
					c.getAssistant().sendFrame34P2(trollPuzzle[d], v, 6980, 1); // 11126
		...
		case 3:
			for (int i = 0; i < 24; i++) {
				...
				if (treePuzzle[d] != 0) {
					c.getAssistant().sendFrame34P2(treePuzzle[d], v, 6980, 1); // 11126
		...
		c.getAssistant().sendFrame34P2(puzzleSlot[4], 4, 6980, 1); // 11126
	}
	...
	public void clickPuzzle(int item) {
		...
		case 2795: // castle
			...
			c.getAssistant().showInterface(6976);
			break;
		case 3576: // troll
			if (openedAlready == false) {
				openPuzzle(2);
	...
	public void clearPuzzle() {
		for (int i = 0; i < 25; i++) {
			c.getAssistant().sendFrame34P2(-1, i, 6980, 0); // 11126
			puzzleSlot[i] = -1;
		}
		int z = 0;
		switch (puzzleId) {
	...
	public void clues(int clueScroll) {
		...
		case 2687:// lvl 2
			...
			break;
		case 2688:// lvl2
			c.getAssistant().showInterface(4305);
			break;
		case 2689:// lvl2
			c.getAssistant().showInterface(9632);
			break;
		case 2690:// lvl 2
		...
		case 2693:// lvl3
			c.getAssistant().showInterface(7113);
			break;
		case 2694:// lvl 3
		...
		case 2697:// lvl 3
			...
			break;
		case 2698:// lvl3
			c.getAssistant().showInterface(6994);
			break;
		case 2699:// lvl 3
