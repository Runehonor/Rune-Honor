package com.runehonor.game.minigames.castlewars;
...
public class CastleWars {
    ...
    public static void addToWaitRoom(Client player, int team) {
        if (player == null) {
            ...
            return;
        } else if (player.petSummoned) { 
        	player.sendMessage("You may not bring pets into the arena.");
        	return;
        }
    ...
    public static void toWaitingRoom(Client player, int team) {
        ...
        if (team == 1) {
            ...
            if (getZammyPlayers() >= getSaraPlayers() || getSaraPlayers() == 0) {
                ...
                player.getAssistant().movePlayer(WAIT_ROOM[team - 1][0] + Misc.random(5), WAIT_ROOM[team - 1][1] + Misc.random(5), 0);
            }
        } else if (team == 2) {
    ...
    public static void returnFlag(Client player, int wearItem) {
        ...
        switch (team) {
            case 1:
                if (wearItem == SARA_BANNER) {
                    ...
                    objectId = 4902;
                    objectTeam = 0;
                    player.sendMessage("returned the Saradomin flag!");
                } else {
                    objectId = 4903;
                    objectTeam = 1;
                    setZammyFlag(0);
            ...
            case 2:
                if (wearItem == ZAMMY_BANNER) {
                    ...
                    objectId = 4903;
                    objectTeam = 1;
                    player.sendMessage("returned the Zamorak flag!");
                } else {
                    objectId = 4902;
                    objectTeam = 0;
                    setSaraFlag(0);
        ...
        player.getAssistant().createPlayerHints(10, -1);
    ...
    public static void pickupFlag(Client player) {
        ...
        while (iterator.hasNext()) {
            ...
            teamPlayer.getAssistant().createObjectHints(player.objectX, player.objectY, 170, -1);
	...
	public static void createHintIcon(Client player, int t) {
        ...
        while (iterator.hasNext()) {
            ...
            teamPlayer.getAssistant().createPlayerHints(10, -1); 
            if (gameRoom.get(teamPlayer) == t) {
                System.out.println("created hint icons for playername " + teamPlayer.playerName + " and team number: " + t);
                teamPlayer.getAssistant().createPlayerHints(10, player.playerId);
                teamPlayer.getAssistant().requestUpdates();
            }
        }
    }
    ...
    public static void createFlagHintIcon(int x, int y) {
        ...
        while (iterator.hasNext()) {
            Client teamPlayer = (Client) iterator.next();
            teamPlayer.getAssistant().createObjectHints(x, y, 170, 2);
        }
    }
    ...
    public static void leaveWaitingRoom(Client player) {
        ...
        if (waitingRoom.containsKey(player)) {
            ...
            player.getAssistant().movePlayer(2439 + Misc.random(4), 3085 + Misc.random(5), 0);
            return;
        }
        player.getAssistant().movePlayer(2439 + Misc.random(4), 3085 + Misc.random(5), 0);
            ...
            if (gameStartTimer <= 10) {
				...
				for (int j = 0; j < PlayerHandler.players.length; j++) {
					if (PlayerHandler.players[j] != null) {
						...
						c2.sendMessage("<img=7><col=CC5200>News: The next game of Castle Wars will begin shortly!");
						messageSent = true;
					}
				}
            }
    ...
    public static void updatePlayers() {
        ...
        while (iterator.hasNext()) {
            ...
            if (player != null) {
                player.getAssistant().sendFrame126("Next game begins in:@red@ " + ((gameStartTimer * 3) + (timeRemaining * 3)) + " @whi@seconds.", 6570);
                player.getAssistant().sendFrame126("Zamorak Players: @red@" + getZammyPlayers() + "@whi@.", 6572);
                player.getAssistant().sendFrame126("Saradomin Players: @blu@" + getSaraPlayers() + "@whi@.", 6664);
                player.getAssistant().walkableInterface(6673);
            }
        }
    }
    ...
    public static void updateCastleWarsScreen(Client c) {
    	if (isInCw(c)) {
    		c.getAssistant().walkableInterface(11146);
			c.getAssistant().sendFrame126(timeRemaining * 3 + " seconds", 11155);
			...
			c.getAssistant().sendFrame87(getTeamNumber(c) == 2 ? 377 : 378, config);
			...
			c.getAssistant().sendFrame87(getTeamNumber(c) == 2 ? 378 : 377, config);
		}
	}
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.autocasting) { // magic
					...
					if (c.MAGIC_SPELLS[c.spellId][4] > 0) {
						c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, 78, c.MAGIC_SPELLS[c.spellId][4], c.getCombat().getStartHeight(), c.getCombat().getEndHeight(), 0, c.getCombat().getStartDelay());
					}
					...
					CycleEventHandler.addEvent(c, new CycleEvent() {
						...
						public void execute(CycleEventContainer container2) {
							try {
								c.getAssistant().createPlayersStillGfx(failMage ? 85 : c.MAGIC_SPELLS[spellId][5], c.objectX, c.objectY, 0, 1);
							} catch (Exception e) {
								container2.stop();
							}
						}
					...
					offY = (pX - oX) * -1;
					try {
						c.getAssistant().createPlayersProjectile(pX, pY, offX, offY, 50, c.getCombat().getProjectileSpeed(), c.getCombat().getRangeProjectileGFX(), 43, 31, 0, c.getCombat().getProjectileShowDelay());
					} catch (Exception e) {
					}
    ...
    public static void updateInGamePlayers() {
        if (getSaraPlayers() > 0 && getZammyPlayers() > 0) {
            ...
            while (iterator.hasNext()) {
                ...
                player.getAssistant().walkableInterface(11146);
                player.getAssistant().sendFrame126("Zamorak = " + scores[1], 11147);
                player.getAssistant().sendFrame126(scores[0] + " = Saradomin", 11148);
                player.getAssistant().sendFrame126(timeRemaining * 3 + " secs", 11155);
                config = (2097152 * saraFlag);
                player.getAssistant().sendFrame87(378, config);
                config = (2097152 * zammyFlag); //flags 0 = safe 1 = taken 2 = dropped
                player.getAssistant().sendFrame87(377, config);
            }
        }
    }
    ...
    public static void startGame() {
        ...
        while (iterator.hasNext()) {
            ...
            player.getAssistant().walkableInterface(-1);
            player.getAssistant().movePlayer(GAME_ROOM[team - 1][0] + Misc.random(3), GAME_ROOM[team - 1][1] - Misc.random(3), 1);
            player.getAssistant().movePlayer(GAME_ROOM[team - 1][0] + Misc.random(3), GAME_ROOM[team - 1][1] - Misc.random(3), 1);
	...
	public static void endGame() {
        ...
        while (iterator.hasNext()) {
    		...
    		for (int i = 0; i < 7; i++) {
    			...
    			player.getAssistant().refreshSkill(i);
    		}
    		player.isDead = false;
            player.cwGames++;
            player.getAssistant().movePlayer(2440 + Misc.random(3), 3089 - Misc.random(3), 0);
    ...
    public static void resetGame() {
    	GameServer.objectManager.removeGameObjects(1);
    ...
    public static void removePlayerFromCw(Client player) {
        ...
        if (gameRoom.containsKey(player)) {
            ...
            deleteGameItems(player);
            player.getAssistant().movePlayer(2440, 3089, 0);
