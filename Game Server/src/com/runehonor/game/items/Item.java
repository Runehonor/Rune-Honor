...
public class Item {
   ...
   // private static String[] fullmask = {"helm (t)","full helm","mask","Verac's helm","Guthan's helm","Karil's coif","mask","Torag's helm", "Void", "sallet"};
	...
	public static String getItemName(int id) {
		for (int j = 0; j < GameServer.itemHandler.ItemList.length; j++) {
			if (GameServer.itemHandler.ItemList[j] != null)
				if (GameServer.itemHandler.ItemList[j].itemId == id)
					return GameServer.itemHandler.ItemList[j].itemName;
		}
		return null;
	}
