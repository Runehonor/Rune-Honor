...
public class ItemAssistant {
	...
	**/
	...
	public void writeBonus() {
		...
		for (int i = 0; i < c.playerBonus.length; i++) {
			...
			c.getAssistant().sendFrame126(send, (1675+i+offset));
		}
	...
	**/
	...
	public void dropAllItems() {
		...
		for(int i = 0; i < c.playerItems.length; i++) {
			if(o != null) {
				if (tradeable(c.playerItems[i] - 1)) {
					GameServer.itemHandler.createGroundItem(o, c.playerItems[i] -1, c.getX(), c.getY(), c.playerItemsN[i], c.killerId);
				} else {
					if (specialCase(c.playerItems[i] - 1))
						GameServer.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerItems[i]-1), c.killerId);
					GameServer.itemHandler.createGroundItem(c, c.playerItems[i] -1, c.getX(), c.getY(), c.playerItemsN[i], c.playerId);
				}
			} else {
				GameServer.itemHandler.createGroundItem(c, c.playerItems[i] -1, c.getX(), c.getY(), c.playerItemsN[i], c.playerId);
			}
		...
		for(int e = 0; e < c.playerEquipment.length; e++) {
			if(o != null) {
				if (tradeable(c.playerEquipment[e])) {
					GameServer.itemHandler.createGroundItem(o, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.killerId);
				} else {
					if (specialCase(c.playerEquipment[e]))
						GameServer.itemHandler.createGroundItem(o, 995, c.getX(), c.getY(), getUntradePrice(c.playerEquipment[e]), c.killerId);
					GameServer.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.playerId);
				}
			} else {
				GameServer.itemHandler.createGroundItem(c, c.playerEquipment[e], c.getX(), c.getY(), c.playerEquipmentN[e], c.playerId);
			}
		...
		if(o != null) {	
			GameServer.itemHandler.createGroundItem(o, 526, c.getX(), c.getY(), 1, c.killerId);
		}	
	}
		 ...
		 for (int j = 0; j < c.playerEquipment.length; j++) {
			if (c.playerEquipment[j] > -1) {
				for ( int k = 0; k < c.playerBonus.length; k++)
					c.playerBonus[k] += GameServer.itemHandler.ItemList[c.playerEquipment[j]].Bonuses[k];
				} 
			} 
		}
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getAssistant().sendFrame126(WeaponName, 5857);
		} else if (WeaponName.endsWith("whip") || WeaponName.endsWith("byssal tentacle")) {
			c.setSidebarInterface(0, 12290); //flick, lash, deflect
			c.getAssistant().sendFrame246(12291, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 12293);
			...
			c.getAssistant().sendFrame246(1765, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 1767);
			...
			c.getAssistant().sendFrame246(329, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 331);
			...
			c.getAssistant().sendFrame246(4447, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 4449);
			...
			c.getAssistant().sendFrame246(2277, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 2279);
		} else if (WeaponName2.startsWith("pickaxe")) {
			c.setSidebarInterface(0, 5570); //spike, impale, smash, block
			c.getAssistant().sendFrame246(5571, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 5573);
		} else if (WeaponName2.startsWith("axe") || WeaponName2.startsWith("battleaxe")) {
			c.setSidebarInterface(0, 1698); //chop, hack, smash, block
			c.getAssistant().sendFrame246(1699, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 1701);
		} else if (WeaponName2.startsWith("halberd")) {
			c.setSidebarInterface(0, 8460); //jab, swipe, fend
			c.getAssistant().sendFrame246(8461, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 8463);
			...
			c.getAssistant().sendFrame246(4680, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 4682);
		} else if (WeaponName2.toLowerCase().contains("mace")){
			c.setSidebarInterface(0, 3796);
			c.getAssistant().sendFrame246(3797, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 3799);

		} else if (c.playerEquipment[c.playerWeapon] == 4153) {
			c.setSidebarInterface(0, 425); //war hamer equip.
			c.getAssistant().sendFrame246(426, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 428);		
		} else {
			c.setSidebarInterface(0, 2423); //chop, slash, lunge, block
			c.getAssistant().sendFrame246(2424, 200, Weapon);
			c.getAssistant().sendFrame126(WeaponName, 2426);
		}
	...
	**/
	...
	public void addSpecialBar(int weapon) {
		switch(weapon) {
			...
			case 4151: // whip
			case 700:
			c.getAssistant().sendFrame171(0, 12323);
			...
			case 11696:
			case 10887:
			c.getAssistant().sendFrame171(0, 7574); 
			specialAmount(weapon, c.specAmount, 7586);
			break;
			
			case 1434: // dragon mace
			c.getAssistant().sendFrame171(0, 7624);
			specialAmount(weapon, c.specAmount, 7636);
			break;
			
			default:
			c.getAssistant().sendFrame171(1, 7624); // mace interface
			c.getAssistant().sendFrame171(1, 7474); // hammer, gmaul
			c.getAssistant().sendFrame171(1, 7499); // axe
			c.getAssistant().sendFrame171(1, 7549);  // bow interface
			c.getAssistant().sendFrame171(1, 7574); // sword interface
			c.getAssistant().sendFrame171(1, 7599); // scimmy sword interface, for most swords
			c.getAssistant().sendFrame171(1, 8493);
			c.getAssistant().sendFrame171(1, 12323); // whip interface
			break;		
		}
	}
	...
	**/
	...
	public void specialAmount(int weapon, double specAmount, int barId) {
		c.specBarId = barId;
		c.getAssistant().sendFrame70(specAmount >= 100 ? 500 : 0, 0, (--barId));
        c.getAssistant().sendFrame70(specAmount >= 90 ? 500 : 0, 0, (--barId));
        c.getAssistant().sendFrame70(specAmount >= 80 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 70 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 60 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 50 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 40 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 30 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 20 ? 500 : 0, 0, (--barId));
		c.getAssistant().sendFrame70(specAmount >= 10 ? 500 : 0, 0, (--barId));	
		updateSpecialBar();
	...
	**/
	...
	public void updateSpecialBar() {
		if(c.usingSpecial && c.playerEquipment[c.playerWeapon] != 15050) {
			c.getAssistant().sendFrame126("@yel@ Special Attack ("+(int)c.specAmount+"%)", c.specBarId);
		} else { 
			c.getAssistant().sendFrame126("@bla@ Special Attack ("+(int)c.specAmount+"%)", c.specBarId);
		}
	}
			...
			if(c.playerItems[slot] == (wearID+1)) {				
				...
				*/
				...
				if(Constants.itemRequirements) {
					if(GameServer.itemHandler.ItemList[wearID].req[1] > 0) {
						if(c.getAssistant().getLevelForXP(c.playerXP[1]) < GameServer.itemHandler.ItemList[wearID].req[1]) {
							c.sendMessage("You need a defence level of "+GameServer.itemHandler.ItemList[wearID].req[1]+" to wear this item.");
							canWearItem = false;
						}
					}
					if(GameServer.itemHandler.ItemList[wearID].req[4] > 0) {
						if(c.getAssistant().getLevelForXP(c.playerXP[4]) < GameServer.itemHandler.ItemList[wearID].req[4]) {
							c.sendMessage("You need a range level of "+GameServer.itemHandler.ItemList[wearID].req[4]+" to wear this item.");
							canWearItem = false;
						}
					}
					if(GameServer.itemHandler.ItemList[wearID].req[6] > 0) {
						if(c.getAssistant().getLevelForXP(c.playerXP[6]) < GameServer.itemHandler.ItemList[wearID].req[6]) {
							c.sendMessage("You need a magic level of "+GameServer.itemHandler.ItemList[wearID].req[6]+" to wear this item.");
							canWearItem = false;
						}
					}
					if(GameServer.itemHandler.ItemList[wearID].req[0] > 0) {
						if(c.getAssistant().getLevelForXP(c.playerXP[0]) < GameServer.itemHandler.ItemList[wearID].req[0]) {
							c.sendMessage("You need an attack level of "+GameServer.itemHandler.ItemList[wearID].req[0]+" to wear this item.");
							canWearItem = false;
						}
					}
					if(GameServer.itemHandler.ItemList[wearID].req[2] > 0) {
						if(c.getAssistant().getLevelForXP(c.playerXP[2]) < GameServer.itemHandler.ItemList[wearID].req[2]) {
							c.sendMessage("You need a strength level of "+GameServer.itemHandler.ItemList[wearID].req[2]+" to wear this item.");
							canWearItem = false;
						}
					}
				}
	...
	**/
	...
	public void dropArrowNpc() {
		...
		if(Misc.random(10) >= 4) {
			if (GameServer.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY) == 0) {
				GameServer.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, 1, c.getId());
			} else if (GameServer.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY) != 0) {
				int amount = GameServer.itemHandler.itemAmount(c.playerName, c.rangeItemUsed, enemyX, enemyY);
				GameServer.itemHandler.removeGroundItem(c, c.rangeItemUsed, enemyX, enemyY, false);
				GameServer.itemHandler.createGroundItem(c, c.rangeItemUsed, enemyX, enemyY, amount+1, c.getId());
			}		
		}
	}	
		...
		for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
			if (GameServer.itemHandler.ItemList[i] != null) {
				if (GameServer.itemHandler.ItemList[i].itemId == ItemID) {
					return GameServer.itemHandler.ItemList[i].itemName;
				}
			}
		}
		return "Unarmed";
	...
	public int getItemId(String itemName) {
		for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
			if (GameServer.itemHandler.ItemList[i] != null) {
				if (GameServer.itemHandler.ItemList[i].itemName.equalsIgnoreCase(itemName)) {
					return GameServer.itemHandler.ItemList[i].itemId;
				}
			}
		}
		return -1;
	...
	public int getUnnotedItem(int ItemID) {
		...
		for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
			if (GameServer.itemHandler.ItemList[i] != null) {
				if (GameServer.itemHandler.ItemList[i].itemId == ItemID) {
					NotedName = GameServer.itemHandler.ItemList[i].itemName;
				}
			}
		}
		for (int i = 0; i < Constants.ITEM_LIMIT; i++) {
			if (GameServer.itemHandler.ItemList[i] != null) {
				if (GameServer.itemHandler.ItemList[i].itemName == NotedName) {
					if (GameServer.itemHandler.ItemList[i].itemDescription.startsWith("Swap this note at any bank for a") == false) {
						NewID = GameServer.itemHandler.ItemList[i].itemId;
						break;
					}
				}
			}
