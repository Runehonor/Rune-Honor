...
public class JewelryMaking extends CraftingData {
	...
	private final static int[][] RINGS =	{ // Ring, Gem, Level, XP
		...
		public static void mouldInterface(Client c) {
			c.getAssistant().showInterface(4161);
		/* Rings */
			if(c.getItems().playerHasItem(1592, 1)) {
				for(int i = 0; i < MOULD_INTERFACE_IDS[0].length; i++) {
					c.getAssistant().sendFrame34(MOULD_INTERFACE_IDS[0][i], i, 4233, 1);
				}
				c.getAssistant().sendFrame34(1643, 4, 4233, 1);
				c.getAssistant().sendFrame126("", 4230);
				c.getAssistant().sendFrame246(4229, 0, -1);
			} else {
				c.getAssistant().sendFrame246(4229, 120, 1592);
				for(int i = 0; i < MOULD_INTERFACE_IDS[0].length; i++) {
					c.getAssistant().sendFrame34(-1, i, 4233, 1);
				}
				c.getAssistant().sendFrame126("You need a ring mould to craft rings.", 4230);
			}
		/* Necklace */
			if(c.getItems().playerHasItem(1597, 1)) {
				for(int i = 0; i < MOULD_INTERFACE_IDS[1].length; i++) {
					c.getAssistant().sendFrame34(MOULD_INTERFACE_IDS[1][i], i, 4239, 1);
				}
				c.getAssistant().sendFrame34(1662, 4, 4239, 1);
				c.getAssistant().sendFrame246(4235, 0, -1);
				c.getAssistant().sendFrame126("", 4236);
			} else {
				c.getAssistant().sendFrame246(4235, 120, 1597);
				c.getAssistant().sendFrame126("You need a necklace mould to craft necklaces", 4236);
				for(int i = 0; i < MOULD_INTERFACE_IDS[1].length; i++) {
					c.getAssistant().sendFrame34(-1, i, 4239, 1);
				}
			}
		/* Amulets */
			if(c.getItems().playerHasItem(1595, 1)) {
				for(int i = 0; i < MOULD_INTERFACE_IDS[2].length; i++) {
					c.getAssistant().sendFrame34(MOULD_INTERFACE_IDS[2][i], i, 4245, 1);
				}
				c.getAssistant().sendFrame34(1681, 4, 4245, 1);
				c.getAssistant().sendFrame246(4241, 0, -1);
				c.getAssistant().sendFrame126("", 4242);
			} else {
				c.getAssistant().sendFrame246(4241, 120, 1595);
				c.getAssistant().sendFrame126("You need a amulet mould to craft necklaces", 4242);
				for(int i = 0; i < MOULD_INTERFACE_IDS[2].length; i++) {
					c.getAssistant().sendFrame34(-1, i, 4245, 1);
				}
			}
		}
		...
		public static void mouldItem(Client c, int item, int amount) {
			...
			if(c.playerLevel[Player.playerCrafting] >= lvl) {
				...
				while((done < amount) && (ItemAssistant.getItemName(gem).toLowerCase().contains("unarmed") && c.getItems().playerHasItem(GOLD_BAR, 1) ||
					...
					c.getAssistant().addSkillXP(xp * Constants.CRAFTING_EXPERIENCE, Player.playerCrafting);
					c.getAssistant().refreshSkill(Player.playerCrafting);
	...
	public static void stringAmulet(final Client c, final int itemUsed, final int usedWith) {
		...
		for (final amuletData a : amuletData.values()) {
			if (amuletId == a.getAmuletId()) {
				...
				c.getAssistant().addSkillXP(4, 12);
