...
import com.runehonor.game.players.Patch;
...
public class Farming {
	...
	public void setPatchInfo(int produce, int xp) {
		...
		if (getNearestFreePatch() > -1) {
			if (c.getAssistant().itemUsedInRegion(2809, 2812, 3335, 3338)) { //Entrana patch
	...
	private void handleFarming(Farming_Data f) {
		if (c.playerLevel[Player.playerFarming] >= f.getReq()) {
			if (c.getItems().playerHasItem(f.getSeed(), 1)) {
				...
				c.getAssistant().addSkillXP(f.getPlantXP(), Player.playerFarming);
				c.getAssistant().refreshSkill(Player.playerFarming);
	...
	public int getPatch() {
		for (int i = 0; i < p.length; i++) {
			if (c.getAssistant().itemUsedInRegion(p[i].minX, p[i].maxX, p[i].minY, p[i].maxY)) {
				return i;
			}
		}
		return -1;
		...
		if (p != null) {
			if (getPatch() > -1) {
				if (p[getPatch()].getProduceAmount() > 0) {
					c.getAssistant().handleObjectRegion(-1, p[getPatch()].minX, p[getPatch()].minY, p[getPatch()].maxX, p[getPatch()].maxY);
					c.getAssistant().object(8143, p[getPatch()].produceSpot[0], p[getPatch()].produceSpot[1], -1, 10);
				} else {
					c.getAssistant().handleObjectRegion(8174, p[getPatch()].getMinX(), p[getPatch()].getMinY(), p[getPatch()].getMaxX(), p[getPatch()].getMaxY());
	...
	public void pickHerb() {
		if (p[getPatch()].getProduceAmount() > 0) {
			if (c.getItems().freeSlots() > 0) {
				...
				c.getAssistant().addSkillXP(p[getPatch()].getPickXP(), Player.playerFarming);
