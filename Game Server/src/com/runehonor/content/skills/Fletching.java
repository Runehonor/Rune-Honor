		...
		if (arr != null) {
			if (c.playerLevel[Player.playerFletching] >= arr.getLevelReq()) {
				...
				if (c.getItems().getItemCount(arr.getItem1()) >= 15 && c.getItems().getItemCount(arr.getItem2()) >= 15) {
					...
					c.getAssistant().addSkillXP(arr.getXp()*Constants.FLETCHING_EXPERIENCE, Player.playerFletching);
		...
		if (bolts != null) {
			if (c.playerLevel[Player.playerFletching] >= bolts.getLevelReq()) {
				...
				if (c.getItems().getItemCount(bolts.getItem1()) >= 10 && c.getItems().getItemCount(bolts.getItem2()) >= 10) {
					...
					c.getAssistant().addSkillXP(bolts.getXp()*Constants.FLETCHING_EXPERIENCE, Player.playerFletching);
			...
			if (id == 52) {
				...
				for (int i = 0; i < logArray.length; i++)
					if (c.getItems().playerHasItem(logArray[i])) {
						...
						c.getAssistant().addSkillXP(fle.getXp()*amount2*Constants.FLETCHING_EXPERIENCE, Player.playerFletching);
	...
	static public void openFletching(Client c, int item) {
		for (int i = 0; i < ifItems.length; i++) {
			if (ifItems[i][0] == item) {
				...
				c.getAssistant().sendFrame126("What would you like to make?", 8879);
				c.getAssistant().sendFrame246(8884, 250, ifItems[i][1]); // middle
				c.getAssistant().sendFrame246(8883, 250, ifItems[i][2]); // left picture
				//c.getPA().sendFrame246(8885, 250, 52); // right pic
				c.getAssistant().sendFrame126("Shortbow", 8889);
				c.getAssistant().sendFrame126("Longbow", 8893);
				if(ifItems[i][0] == 1511) {
					c.getAssistant().sendFrame246(8885, 250, 52); // right pic
					c.getAssistant().sendFrame126("Arrow Shafts", 8897);
				} else {
					c.getAssistant().sendFrame246(8885, 250, -1); // right pic
					c.getAssistant().sendFrame126(" ", 8897);
				}
			}
		}
		c.log = item;
						...
						if (c.getItems().playerHasItem(craftingVariables[i][0],
							...
							c.getAssistant().addSkillXP(
									craftingVariables[i][4]
											* Constants.FLETCHING_EXPERIENCE,
