...
public class Tanning extends CraftingData {
	...
	public static void sendTanningInterface(final Client c) {
		c.getAssistant().showInterface(14670);
		for (final tanningData t : tanningData.values()) {
			c.getAssistant().itemOnInterface(t.getItemFrame(), 250, t.getLeatherId());
			c.getAssistant().sendFrame126(t.getName(), t.getNameFrame());
			if (c.getItems().playerHasItem(995, t.getPrice())) {
				c.getAssistant().sendFrame126("@gre@Price: "+ t.getPrice(), t.getCostFrame());
			} else {
				c.getAssistant().sendFrame126("@red@Price: "+ t.getPrice(), t.getCostFrame());
			}
		}
	}
