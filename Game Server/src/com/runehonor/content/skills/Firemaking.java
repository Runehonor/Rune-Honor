...
import com.runehonor.world.Tile;
		...
		for(int i = 0; i < logsdata.length; i++) {
			if((requiredItem(c, useWith) && withUse == logsdata[i][0] || useWith == logsdata[i][0] && requiredItem(c, withUse))) {
				...
				if (System.currentTimeMillis() - c.lastFire > 1200) {
					...
					GameServer.itemHandler.createGroundItem(c, log, coords[0], coords[1], 1, c.getId());

					CycleEventHandler.addEvent(c, new CycleEvent() {
						@Override
						public void execute(CycleEventContainer container) {
							new Object(fire, coords[0], coords[1], c.heightLevel, 0, 10, fire, -1, 1);
							GameServer.itemHandler.removeGroundItem(c, log, coords[0], coords[1], false);
					...
					@Override
						public void execute(CycleEventContainer container) {
							...
							GameServer.itemHandler.createGroundItem(c, 592, coords[0], coords[1], 1, c.getId());
							container.stop();
						}
						@Override
						public void stop() {
					...
					c.getAssistant().addSkillXP(logsdata[i][2] * Constants.FIREMAKING_EXPERIENCE, 11);
