...
public class Agility {
	...
	public void brimhavenMonkeyBars(Client c, String Object, int level, int x, int y, int a, int b, int xp)
	{
		...
		if (c.absX == a && c.absY == b) { 
			c.getAssistant().walkTo3(x, y);
			
			c.getAssistant().addSkillXP(xp, Player.playerAgility);
			
			c.getAssistant().refreshSkill(Player.playerAgility);
		}
	}
	...
	public void doWildernessEntrance(final Client c) {
		if (System.currentTimeMillis() - c.foodDelay < 2000) {
			...
			c.getAgil().wildernessEntrance(c, "Door", 1, 0, +15, 2998, 3917, 40 * Constants.AGILITY_EXPERIENCE);
	...
	public void gnomePipe(Client c, String pipe, int level, int a, int b, int x, int y, int add, int amount, int xp)
	{
		...
		if (c.absX == a && c.absY == b)
		{
					...
					&& c.ag5 >= 1 && c.ag6 >= 1)
			{
				...
				c.getAssistant().addSkillXP(360, Player.playerAgility);
				c.getAssistant().refreshSkill(Player.playerAgility);
	...
	public void doGnomeLog(final Client c) {
			if (System.currentTimeMillis() - c.foodDelay < 2000) {
				...
				c.updateRequired = true;
				c.appearanceUpdateRequired = true;
				c.getAgil().gnomeLog(c, "Log", 1, 0, -7, 2474, 3436, 8 * Constants.AGILITY_EXPERIENCE);
				c.ag1 = 1;
				c.foodDelay = System.currentTimeMillis();
				CycleEventHandler.addEvent(c, new CycleEvent() {
					@Override
	...
	public void doGnomeNet1(final Client c) {
		...
		return;
			...
			c.getAgil().gnomeNet(c, "Net", 1, 2471, 3426, 1, 2471, 3424, 828, 8 * Constants.AGILITY_EXPERIENCE);
			c.getAgil().gnomeNet(c, "Net", 1, 2472, 3426, 1, 2472, 3424, 828, 8 * Constants.AGILITY_EXPERIENCE);
			c.getAgil().gnomeNet(c, "Net", 1, 2473, 3426, 1, 2473, 3424, 828, 8 * Constants.AGILITY_EXPERIENCE);
			c.getAgil().gnomeNet(c, "Net", 1, 2474, 3426, 1, 2474, 3424, 828, 8 * Constants.AGILITY_EXPERIENCE);
			c.getAgil().gnomeNet(c, "Net", 1, 2475, 3426, 1, 2475, 3424, 828,8 * Constants.AGILITY_EXPERIENCE);
			c.getAgil().gnomeNet(c, "Net", 1, 2476, 3426, 1, 2476, 3424, 828, 8 * Constants.AGILITY_EXPERIENCE);
			c.ag2 = 1;
			c.foodDelay = System.currentTimeMillis();
	}
	...
	public void doGnomeBranch1(final Client c) {
		if (System.currentTimeMillis() - c.foodDelay < 2000) {
			return;
				...
				c.stopMovement();
				c.startAnimation(828);
				c.getAgil().gnomeBranch(c, "Branch", 1, 2473, 3420, 2, 2473, 3423, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeBranch(c, "Branch", 1, 2473, 3420, 2, 2474, 3422, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeBranch(c, "Branch", 1, 2473, 3420, 2, 2472, 3422, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.ag3 = 1;
				c.foodDelay = System.currentTimeMillis();
	}
	...
	public void doGnomeBranch2(final Client c) {
		if (System.currentTimeMillis() - c.foodDelay < 2000) {
			return;
				...
				c.getAgil().gnomeBranch(c, "Branch", 1, 2486, 3420, 0, 2485, 3419, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeBranch(c, "Branch", 1, 2486, 3420, 0, 2485, 3420, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeBranch(c, "Branch", 1, 2486, 3420, 0, 2486, 3420, 828, 5 * Constants.AGILITY_EXPERIENCE);
				c.ag5 = 1;
				c.foodDelay = System.currentTimeMillis();
	}
	...
	public void doGnomeNet2(final Client c) {
		if (System.currentTimeMillis() - c.foodDelay < 2000) {
			return;
				...
				c.stopMovement();
				c.startAnimation(828);
				c.getAgil().gnomeNet(c, "Net", 1, 2483, 3425, 0, 2483, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeNet(c, "Net", 1, 2484, 3425, 0, 2484, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeNet(c, "Net", 1, 2485, 3425, 0, 2485, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeNet(c, "Net", 1, 2486, 3425, 0, 2486, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeNet(c, "Net", 1, 2487, 3425, 0, 2487, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.getAgil().gnomeNet(c, "Net", 1, 2488, 3425, 0, 2488, 3427, 828, 8 * Constants.AGILITY_EXPERIENCE);
				c.ag6 = 1;
	...
	public void doGnomeRope(final Client c) {
				...
				c.updateRequired = true;
				c.appearanceUpdateRequired = true;
				c.getAgil().gnomeLog(c, "Log", 1, +6, 0, 2477, 3420, 7 * Constants.AGILITY_EXPERIENCE);
				c.ag4 = 1;
				c.foodDelay = System.currentTimeMillis();
				CycleEventHandler.addEvent(c, new CycleEvent() {
					@Override
	...
	public void doGnomePipe1(final Client c) {
			...
			c.updateRequired = true;
			c.appearanceUpdateRequired = true;
			c.getAgil().gnomePipe(c, "Pipe", 1, 2484, 3430, 0, +7, 2996, 10, 47 * Constants.AGILITY_EXPERIENCE);
			c.foodDelay = System.currentTimeMillis();
			CycleEventHandler.addEvent(c, new CycleEvent() {
				@Override
				public void execute(CycleEventContainer container) {
	...
	public void doGnomePipe2(final Client c) {
		...
		c.updateRequired = true;
		c.appearanceUpdateRequired = true;
		c.getAgil().gnomePipe(c, "Pipe", 1, 2487, 3430, 0, +7, 2996, 10, 47 * Constants.AGILITY_EXPERIENCE);
		c.foodDelay = System.currentTimeMillis();
		CycleEventHandler.addEvent(c, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
