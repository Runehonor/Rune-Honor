...
import com.runehonor.content.skills.misc.SkillHandler;
	...
	c.turnPlayerTo(c.objectX, c.objectY);
		...
		CycleEventHandler.addEvent(5, c, new CycleEvent() {
			...
			c.turnPlayerTo(c.objectX, c.objectY);
				...
				c.getAssistant().addSkillXP(5 * Constants.MINING_EXPERIENCE, Player.playerMining);
				...
				CycleEventHandler.addEvent(5, c, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						...
						if(c.playerSkillProp[14][1] > 0) {
							c.getAssistant().addSkillXP(c.playerSkillProp[14][1] * Constants.MINING_EXPERIENCE, Player.playerMining);
		...
		c.turnPlayerTo(c.objectX, c.objectY);
			...
			c.startAnimation(getAnimation(c));
					...
					CycleEventHandler.addEvent(5, c, new CycleEvent() {
						...
						public void execute(CycleEventContainer container) {
							if(object > 0) {
								c.getItems().addItem(c.getAssistant().randomGems(), 1);
								c.sendMessage("You manage to mine a gem.");
								c.startAnimation(getAnimation(c));
							}
							if(object > 0) {
								c.getAssistant().addSkillXP(65 * Constants.MINING_EXPERIENCE, Player.playerMining);
								new Object(451, obX, obY, c.heightLevel, 0, 10, 451, -1, 0);
							}
							if(!hasPickaxe(c)) {
								c.sendMessage("You need a pickaxe to mine this rock.");
