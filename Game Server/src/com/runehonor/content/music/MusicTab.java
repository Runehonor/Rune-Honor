package com.runehonor.content.music;
...
public class MusicTab {
        ...
        public static void loadMusicTab(Client c) {
                       for (int i = 0; i < Music.songs.length; i++) {
	                       if (Music.unlocked[Music.songs[i].array] == true) {
	                       	...
	                       	c.getAssistant().sendColor(Music.songs[i].tabId, 255 << 10 |0  << 5 | 0);
	                       }
                       }
                       c.getAssistant().sendFrame36(18, 1); //set to AUTO. 1 = AUTO 0 = MAN
                       c.getAssistant().sendFrame36(19, 1); //turn on LOOP
        }
        ...
        public static void setToManual(Client c) {
        	c.auto = 0;
    		c.getAssistant().sendFrame36(18, 0); //set to AUTO. 1 = AUTO 0 = MAN
        }
        ...
        public static void handleClick(Client c, int Id) {
	    		...
	    		case 38197:
	    			...
	    			c.getAssistant().sendFrame36(19, setting);
	    			break;
	        	}
            	for (int i = 0; i < music.length; i++) {
                        ...
                        c.getAssistant().sendFrame126(music[i].songName, 4439);
