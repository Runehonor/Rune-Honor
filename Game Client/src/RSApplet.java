...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener, FocusListener, WindowListener {
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if(mouseX > 0 && mouseX < 512 && mouseY > 503 - 165 && mouseY < 503 - 25) {
			int scrollPos = AJ.anInt1089;
			scrollPos -= rotation * 30;		
			if(scrollPos < 0)
				scrollPos = 0;
			if(scrollPos > AJ.anInt1211 - 110)
				scrollPos = AJ.anInt1211 - 110;
			if(AJ.anInt1089 != scrollPos) {
				AJ.anInt1089 = scrollPos;
				AJ.inputTaken = true;
			}
		}
	}
	public void handleInterfaceScrolling(MouseWheelEvent event) {
		...
		int tabInterfaceID = AJ.tabInterfaceIDs[AJ.tabID];
		if (tabInterfaceID != -1) {
			...
			if (mouseX > offsetX + positionX && mouseY > offsetY + positionY && mouseX < offsetX + positionX + width && mouseY < offsetY + positionY + height) {
				...
				AJ.tabAreaAltered = true;
				AJ.needDrawTabArea = true;
			}
		}
		/* Main interface scrolling */
		if (AJ.openInterfaceID != -1) {
			RSInterface rsi = RSInterface.interfaceCache[AJ.openInterfaceID];
			offsetX = 4;
			offsetY = 4;
